...
public class HashApp extends GenericProtocolExtension {
    ...
    //Client callbacks
    private final Map<UUID, Triple<String, Long, Host>> clientIdMapper;
    ...
    public HashApp(Properties properties) throws HandlerRegistrationException, IOException {
        ...
        String NETWORK_PROTO  = properties.getProperty("NETWORK_PROTOCOL");
        String channelName;
        ...
        System.out.println("RUNNING PROTO"+NETWORK_PROTO+" -- "+channelName);

        //This channel has only two events - ClientUp and ClientDown (and, obviously, receiving messages)
        registerChannelEventHandler(channelId, OnMessageConnectionUpEvent.EVENT_ID, this::onClientUp);
        registerChannelEventHandler(channelId, OnConnectionDownEvent.EVENT_ID, this::onClientDown);
    ...
    private void uponRequestMessage(RequestMessage msg, Host host, short sourceProto, int channelId, String connectionID) {
        logger.debug("Request received: " + msg + " from " + host);
        UUID opUUID = UUID.randomUUID();
        clientIdMapper.put(opUUID, Triple.of(connectionID, msg.getOpId(),host));
        Operation op = new Operation(msg.getOpType(), msg.getKey(), msg.getData());
        /**
        ResponseMessage resp;
        if (op.getOpType() == RequestMessage.WRITE) {
            resp = new ResponseMessage(msg.getOpId(), new byte[0]);
        }else {
            resp = new ResponseMessage(msg.getOpId(),msg.getData());
        }
        //Respond
        sendMessage(resp,connectionID);
        if(msg!=null){
            return;
        } **/
        try {
    ...
    private void uponExecuteNotification(ExecuteNotification not, short sourceProto) {
        try {
            ...
            Triple<String, Long,Host> pair = clientIdMapper.remove(not.getOpId());
            if (pair != null) {
                //Generate a response to the client
                ResponseMessage resp;
                if (op.getOpType() == RequestMessage.WRITE)
                    resp = new ResponseMessage(pair.getMiddle(), new byte[0]);
                else
                    resp = new ResponseMessage(pair.getMiddle(), data.getOrDefault(op.getKey(), new byte[0]));
                //Respond
                if(pair.getLeft()==null){
                    sendMessage(resp,pair.getRight());
                }else{
                    sendMessage(resp,pair.getLeft());
                }
                logger.debug("RESPONSE SENT: ID={}",resp.getOpId());
            }
    ...
    private void onClientUp(OnMessageConnectionUpEvent event, int channel) {
        logger.info(event);
    }

    private void onClientDown(OnConnectionDownEvent event, int channel) {
        logger.info(event);
    }
