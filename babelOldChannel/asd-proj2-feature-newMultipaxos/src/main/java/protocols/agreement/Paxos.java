...
public class Paxos extends GenericProtocol {
    ...
    private void processPendingMessage(ProtoMessage msg, Host from) {
        if (msg instanceof AcceptMessage acceptMessage) {
            uponAcceptMessage(acceptMessage, from, (short) -1, -1);
        } else if (msg instanceof PrepareMessage prepareMessage) {
            uponPrepareMessage(prepareMessage, from, (short) -1, -1);
        } else if (msg instanceof ProposeMessage proposeMessage) {
            uponProposeMessage(proposeMessage, from, (short) -1, -1);
        } else if (msg instanceof PromiseMessage promiseMessage) {
            uponPromiseMessage(promiseMessage, from, (short) -1, -1);
        } else {
    ...
    private void uponPromiseMessage(PromiseMessage msg, Host from, short sourceProto, int channelId) {
        log.debug("Received {} from {}", msg, from);
        int instance = msg.getInstance();
        if (instance < currentInstance) {
    ...
    private void uponProposeMessage(ProposeMessage msg, Host from, short sourceProto, int channelId) {
        log.debug("Received {} from {}", msg, from);
        int instance = msg.getInstance();
        if (instance < currentInstance) {
