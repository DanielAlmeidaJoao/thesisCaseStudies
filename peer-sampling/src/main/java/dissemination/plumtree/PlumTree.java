...
import fileStreamingProto.PlumTreeInterface;
...
public class PlumTree extends GenericProtocolExtension implements PlumTreeInterface {
    ...
    private void uponReceiveGossip(GossipMessage msg, Host from, short sourceProto, int channelId,String connectionId) {
        ...
        if(!received.containsKey(msg.getMid())) {
            DeliverReply r = new DeliverReply(msg.getContent());
            r.hash = msg.getMid();
            sendReply(r, msg.getToDeliver());
    ...
    private void uponBroadcast(BroadcastRequest request, short sourceProto) {
        GossipMessage msg = new GossipMessage(request.mid, 0, sourceProto, request.getMsg());
        eagerPush(msg, 0, myself);
        lazyPush(msg, 0, myself);
        DeliverReply r = new DeliverReply(msg.getContent());
        r.hash = msg.getMid();
        sendReply(r, sourceProto);
        received.put(request.mid, msg);
        stored.add(request.mid);
        if (stored.size() > space) {
            int torm = stored.poll();
            received.put(torm, null);
        }
